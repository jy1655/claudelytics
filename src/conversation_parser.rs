...
impl ConversationParser {
    ...
    pub fn parse_conversation(&self, file_path: &Path) -> Result<Conversation> {
        ...
        for (line_idx, line_result) in reader.lines().enumerate() {
            ...
            // If first record is a summary, keep it as metadata.
            if !first_record_seen && record.get("type").and_then(|t| t.as_str()) == Some("summary")
            {
                if let Ok(summary) = serde_json::from_value::<ConversationSummary>(record.clone()) {
            ...
            if let Err(e) = self.parse_message_record(&record, &mut conversation) {
                eprintln!(
                    "Warning: Skipping malformed conversation record at {}:{} ({})",
                    file_path.display(),
                    line_number,
                    e
                );
    ...
    fn parse_message_record(
        ...
        if let Some(message) = record.get("message") {
            ...
            let mut usage = if let Some(usage_data) = message.get("usage") {
                self.parse_usage(usage_data)?
            } else {
                None
            };

            if let Some(ref mut msg_usage) = usage {
                msg_usage.total_cost = Self::extract_cost_usd(record);
            }
                ...
                match content_item.get("type").and_then(|t| t.as_str()) {
                    Some("text") | Some("thinking") => {
                        ...
                        let content_type = content_item
                            ...
                            .unwrap_or("text")
                            .to_string();
                        blocks.push(MessageContentBlock::Text { content_type, text });
                    }
                    Some("tool_use") => {
                        let content_type = content_item
                            .get("type")
                    ...
                    Some("tool_result") => {
                        ...
                        let tool_use_id = content_item
                            .get("tool_use_id")
                            .and_then(|v| v.as_str())
                            .or_else(|| content_item.get("toolUseId").and_then(|v| v.as_str()))
                            .unwrap_or("")
                            .to_string();
                        let content = content_item
                            ...
                            .map(Self::render_content_value)
                            .unwrap_or_default();
                        blocks.push(MessageContentBlock::ToolResult {
                            content_type,
                            tool_use_id,
                            content,
                        });
    ...
    fn extract_cost_usd(record: &serde_json::Value) -> f64 {
        for key in ["costUSD", "total_cost_usd"] {
            if let Some(cost) = record.get(key).and_then(|v| v.as_f64()) {
                return cost;
            }

            if let Some(cost_str) = record.get(key).and_then(|v| v.as_str())
                && let Ok(parsed) = cost_str.parse::<f64>()
            {
                return parsed;
            }
        }
...
mod tests {
    ...
    fn test_parse_conversation_with_summary() {
        ...
        writeln!(file, r#"{{"uuid":"msg2","parentUuid":"msg1","type":"assistant","timestamp":"2024-01-01T12:00:01Z","sessionId":"session1","costUSD":0.0123,"message":{{"role":"assistant","model":"claude-3-opus","content":[{{"type":"text","text":"Hi there!"}}],"usage":{{"input_tokens":10,"output_tokens":5}}}}}}"#).unwrap();
        ...
        assert!((conversation.total_usage.total_cost - 0.0123).abs() < f64::EPSILON);
    }
    ...
    #[test]
    fn test_parse_conversation_skips_malformed_records() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test.jsonl");
        let mut file = File::create(&file_path).unwrap();
        ...
        writeln!(file, r#"{{"uuid":"msg1","parentUuid":null,"type":"user","timestamp":"2024-01-01T12:00:00Z","sessionId":"session1","message":{{"role":"user","content":[{{"type":"text","text":"Hello"}}]}}}}"#).unwrap();
        writeln!(file, r#"{{"uuid":"bad-ts","parentUuid":"msg1","type":"assistant","timestamp":"not-a-timestamp","sessionId":"session1","message":{{"role":"assistant","content":[{{"type":"text","text":"bad"}}]}}}}"#).unwrap();
        writeln!(file, r#"{{"uuid":"msg2","parentUuid":"msg1","type":"assistant","timestamp":"2024-01-01T12:00:01Z","sessionId":"session1","message":{{"role":"assistant","content":[{{"type":"text","text":"Hi there!"}}],"usage":{{"input_tokens":10,"output_tokens":5}}}}}}"#).unwrap();
        ...
        assert!(conversation.summary.is_some());
        assert_eq!(conversation.messages.len(), 2);
        assert_eq!(conversation.messages[0].uuid, "msg1");
        assert_eq!(conversation.messages[1].uuid, "msg2");
    }

    #[test]
    fn test_tool_result_object_content_is_supported() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test.jsonl");
        let mut file = File::create(&file_path).unwrap();

        writeln!(file, r#"{{"uuid":"msg1","parentUuid":null,"type":"assistant","timestamp":"2024-01-01T12:00:00Z","sessionId":"session1","message":{{"role":"assistant","content":[{{"type":"tool_result","tool_use_id":"tool1","content":[{{"type":"text","text":"result line 1"}},{{"type":"text","text":"result line 2"}}]}}]}}}}"#).unwrap();
        ...
        match &conversation.messages[0].content[0] {
            MessageContentBlock::ToolResult { content, .. } => {
                assert!(content.contains("result line 1"));
                assert!(content.contains("result line 2"));
            }
