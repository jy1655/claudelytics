...
mod tests {
    ...
    fn test_burn_rate_calculation() {
        ...
        // Stabilize test timing: ensure active block started > 6 minutes ago
        // so burn-rate threshold (0.1h) is always satisfied.
        for blocks in manager.blocks.values_mut() {
            for block in blocks.iter_mut() {
                if block.is_active {
                    block.start_time = now - Duration::minutes(7);
                    block.end_time = block.start_time + Duration::hours(manager.config.block_hours);
                }
            }
        }
