...
struct Cli {
    ...
    #[arg(
        ...
        long_help = "Launch advanced TUI mode (alias of the enhanced TUI with advanced state)\nFeatures: Command palette, bookmarks, comparisons, conversation tools"
    )]
    advanced_tui: bool,
...
enum Commands {
    ...
    #[command(
        long_about = "Launch advanced TUI mode\n\nThis command is an alias for the enhanced TUI with advanced mode state enabled.\nIt includes command palette, bookmark/comparison workflows, and conversation tools.\n\nEXAMPLE:\n  claudelytics advanced-tui            # Launch advanced TUI mode"
    )]
    AdvancedTui,
    ...
    #[command(
        long_about = "Start an MCP server to expose claudelytics data via the Model Context Protocol\n\nThe MCP server allows other applications to query claudelytics data through\na standardized protocol. Currently, stdio transport is supported.\nHTTP transport is reserved but not yet implemented.\n\nEXAMPLES:\n  claudelytics mcp-server                # Start stdio server\n  claudelytics mcp-server --http 8080    # Reserved (currently returns not-implemented)\n  claudelytics mcp-server --list-tools   # Show available MCP tools\n  claudelytics mcp-server --list-resources # Show available MCP resources"
    )]
    McpServer {
        #[arg(
            long,
            help = "Reserved HTTP server port (not implemented yet)",
            long_help = "Reserved for future HTTP MCP transport. Currently unsupported.\nPassing this option returns a not-implemented error."
        )]
        http: Option<u16>,
        #[arg(
            long,
    ...
    #[command(
        long_about = "Manage the offline pricing cache for model costs\n\nThe pricing cache allows claudelytics to work without internet connection\nby storing pricing data locally. Cache entries remain valid for 7 days.\n\nNOTE:\n  Current --update refreshes cache from built-in pricing data.\n  Online price fetching is planned but not yet implemented.\n\nEXAMPLES:\n  claudelytics pricing-cache           # Show current cache status\n  claudelytics pricing-cache --show    # Show current cache status\n  claudelytics pricing-cache --clear   # Clear the cache\n  claudelytics pricing-cache --update  # Refresh cache from built-in pricing"
    )]
    PricingCache {
        #[arg(
            long,
...
fn run() -> Result<()> {
    ...
    if cli.tui || cli.advanced_tui {
        let tui_mode = if cli.advanced_tui {
            TuiMode::Advanced
        } else {
            TuiMode::Basic
        };
        ...
        save_tui_state(&tui_app, tui_mode).ok();
        return result;
    }
    ...
    match command {
        ...
        Commands::AdvancedTui => {
            ...
            save_tui_state(&tui_app, TuiMode::Advanced).ok();
            result?;
        }
...
fn handle_mcp_server_command(
    ...
    if let Some(port) = http_port {
        anyhow::bail!(
            "MCP HTTP server is not available yet (requested port: {}). Use stdio mode instead.",
            port
        );
    } else {
...
fn handle_conversation_command(
    ...
    let filtered_for_display: Vec<Conversation> = filtered_conversations
        .iter()
        .map(|conv| filter_conversation_for_display(conv, include_thinking, include_tools))
        .collect();
    ...
    if let Some(export_format) = &export {
        let content = match export_format.as_str() {
            ...
            _ => {
                ...
                for conv in &filtered_for_display {
...
fn filter_conversation_for_display(
    conversation: &conversation_parser::Conversation,
    include_thinking: bool,
    include_tools: bool,
) -> conversation_parser::Conversation {
    if include_thinking && include_tools {
    ...
    for message in &mut filtered.messages {
        message.content.retain(|block| match block {
            conversation_parser::MessageContentBlock::Text { content_type, .. } => {
                include_thinking || content_type != "thinking"
            }
            conversation_parser::MessageContentBlock::ToolUse { .. }
            | conversation_parser::MessageContentBlock::ToolResult { .. } => include_tools,
        });
    }
...
fn handle_inspect_command(
    ...
    if json {
        ...
        for (session_path, (usage, last_activity)) in sessions {
            ...
            let session_id = parts.last().copied().unwrap_or("unknown");
