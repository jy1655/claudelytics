...
impl UsageParser {
        ...
        for line in reader.lines() {
            ...
            match serde_json::from_str::<UsageRecord>(&line) {
                Ok(record) => {
                    ...
                    {
                        let session_info = match self.resolve_session_info(&record, file_path) {
                            Ok(value) => value,
                            Err(e) => {
                                eprintln!(
                                    "Warning: Failed to resolve session info for {}: {}",
                                    file_path.display(),
                                    e
                                );
                                continue;
    ...
    fn extract_project_path(&self, file_path: &Path) -> Result<String> {
        ...
        let project_path = relative_path
            ...
            .ok_or_else(|| anyhow::anyhow!("Invalid project path structure"))?;

        Ok(project_path.to_string())
    }

    fn resolve_session_info(&self, record: &UsageRecord, file_path: &Path) -> Result<String> {
        ...
        if let Some(session_id) = record
            ...
            .filter(|s| !s.is_empty())
        {
            return Ok(format!("{}/{}", project_path, session_id));
        }

        // Fallback: derive from path. If this is a subagent transcript without sessionId,
        // use file stem to avoid collapsing all records into a shared "subagents" bucket.
        let fallback_path = self.extract_session_info(file_path)?;
        if fallback_path.ends_with("/subagents")
            && let Some(file_stem) = file_path.file_stem().and_then(|s| s.to_str())
        {
            return Ok(format!("{}/{}", project_path, file_stem));
        }
...
mod tests {
    ...
    #[test]
    fn test_resolve_session_info_prefers_session_id() {
        let temp_dir = TempDir::new().expect("Failed to create temp dir");
        let parser = UsageParser::new(temp_dir.path().to_path_buf(), None, None, None)
            .expect("Failed to create parser");

        let project_dir = temp_dir.path().join("projects").join("test-project");
        fs::create_dir_all(&project_dir).expect("Failed to create test project dir");

        let path = project_dir.join("random-file-name.jsonl");
        create_test_jsonl_file(&project_dir, "random-file-name.jsonl", "");
        ...
        let session_info = parser
            .resolve_session_info(&record, &path)
            .expect("Failed to resolve session info");
        assert_eq!(session_info, "test-project/session-abc-123");
    }

    #[test]
    fn test_resolve_session_info_subagent_fallback_uses_file_stem() {
        let temp_dir = TempDir::new().expect("Failed to create temp dir");
        let parser = UsageParser::new(temp_dir.path().to_path_buf(), None, None, None)
            .expect("Failed to create parser");
        ...
        let subagent_dir = temp_dir
            ...
            .join("test-project")
            .join("subagents");
        fs::create_dir_all(&subagent_dir).expect("Failed to create subagents dir");

        let path = subagent_dir.join("agent-def456.jsonl");
        create_test_jsonl_file(&subagent_dir, "agent-def456.jsonl", "");

        // No sessionId in record -> fallback path logic
        let record: UsageRecord =
            serde_json::from_str(r#"{"message":{"usage":{"input_tokens":1,"output_tokens":1}}}"#)
                .expect("Failed to parse test record");

        let session_info = parser
            .resolve_session_info(&record, &path)
            .expect("Failed to resolve session info");
        assert_eq!(session_info, "test-project/agent-def456");
    }
