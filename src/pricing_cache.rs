...
/// Bump this when built-in fallback pricing is updated within the same app version.
const PRICING_SCHEMA_VERSION: &str = "pricing-v2026-03-06";
...
impl PricingCache {
    fn cache_version() -> String {
        format!("{}+{}", env!("CARGO_PKG_VERSION"), PRICING_SCHEMA_VERSION)
    }
...
mod tests {
    ...
    fn test_model_variations() {
        ...
        assert!(variations.contains(&"claude-sonnet-4".to_string()));

        let variations = CachedPricingFetcher::get_model_variations("opus-4");
        assert!(variations.contains(&"claude-opus-4".to_string()));
    }
