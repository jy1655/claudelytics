...
### Fork Initialization - 2026-03-06

This repository is now maintained as a fork of `nwiizo/claudelytics`.

### ✨ Added
- Added fork lineage and maintenance scope in `README.md`.
- Added `CONTRIBUTING.md` and `SECURITY.md` for open-source collaboration.

### 🔧 Improved
- Updated repository/install references to `jy1655/claudelytics`.
- Updated MCP server metadata to include fork and upstream repository info.
- Refreshed fallback model pricing and model registry for current Claude model lineup.

### 🐛 Fixed
- Improved parser compatibility for newer Claude Code transcript/session structures.
- Corrected pricing cache versioning behavior so updated built-in pricing invalidates stale caches.

## [0.5.2] - 2025-08-15

### ✨ Added
- Added `advanced-tui` command and `--advanced-tui` global flag as advanced launch aliases for TUI mode.
- Added a functional MCP stdio server request loop with JSON-RPC framing support.

### 🔧 Improved
- CLI version output now follows Cargo package version automatically.
- Updated docs/version references to align with `0.5.2`.

### 🐛 Fixed
- `conversation` command now applies `--include-thinking` and `--include-tools` filters in terminal and text/json exports.
- Conversation parser now includes per-message `costUSD` in usage totals.
